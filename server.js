...
const ISP_NEEDS=`你是臺灣大專校院資源教室的 ISP 行政文字助理。依輸入的現況能力摘要、學生優弱勢與現況分析，產生「學生需求評估」。不得使用或推測姓名、障別、診斷、病史、證明、家庭資料、家長期望或自我期望。先辨識真正需要支持的困難，再轉化為具體服務需求，不要只是改寫摘要。使用阿拉伯數字逐點輸出，格式為「1. 服務需求：簡要說明」。相同性質整合，不同需求分點。正常、穩定、與一般學生相當或不適用面向不列入。沒有資料不要補寫。原則上一至六項。若無明顯特殊需求，只輸出「1. 持續追蹤：目前整體適應尚可，暫無明顯特殊服務需求，後續依實際適應情形持續追蹤。」每一點都必須是完整句子，不得在連接詞或介系語句中途結束。只輸出可直接貼入欄位的列點。`;

const ISP_SERVICE=`你是臺灣大專校院資源教室的 ISP 行政文字助理。依學生需求評估、八項現況能力摘要及老師填寫的具體說明，產生「服務評估摘要」。已勾選的支持策略與服務僅用來核對方向，不要逐項羅列。聚焦一至三項核心需求及其對就學、生活適應或人際互動的影響，整合成一段連貫文字，包含整體現況與主要需求、支持重點及後續追蹤方向。僅使用明確提供的事實，不得新增、猜測或推論。尚未實施的服務不可寫成已有成效。原則上80至160個中文字。每一句都必須完整收尾，不得在連接詞或介系語句中途結束。只輸出可直接貼入欄位的文字。`;

const SERVICE_POLISH=`你是臺灣大專校院資源教室的「服務紀錄內容摘述」文字助理。只依老師提供的原文進行潤飾，不得新增原文沒有的學生資訊、診斷、原因、事件、服務、成效、人物、日期或判斷。保留原意、時間、對象、事件順序與程度。請將口語、零碎或不完整的敘述整理成正式、客觀、中性、自然且適合資源教室服務紀錄的繁體中文。可改善語序、用詞、標點與句型，使內容更清楚，但不得把推測寫成事實。若原文有列點或分行，盡量保留原本結構；若原文為一段文字，不要自行新增標題或固定格式。不得回覆「內容已經很好」「無需修改」等說明。每一句都必須完整收尾，不得以連接詞或未完成語句結束。只輸出可直接貼回「內容摘述」欄位的文字，不要標題、前言、引號、Markdown 或額外說明。`;
...
function looksIncomplete(text,finishReason=''){
  ...
  const tail=value.replace(/[\s。！？；!?;]+$/g,'');
  if(/(?:且|並|及|與|或|於|在|因|而|但|故|使|讓|以|若|則|以及|並且|同時|仍|亦|例如|包括|包含|透過|藉由)$/.test(tail))return true;
  if(/(?:於|在)[^，。！？；]{0,14}(?:中|時|期間|方面|情況下)$/.test(tail))return true;
  return false;
}
...
async function callGemini(text,mode,section,model,timeoutMs=12000){
  const p=promptFor(mode,section);
  ...
  try{
    const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`,{
      ...
      signal:c.signal,
      body:JSON.stringify({
        systemInstruction:{parts:[{text:p.instruction}]},
        contents:[{role:'user',parts:[{text:`${p.task}\n\n${text}`}]}],
        generationConfig:{temperature:p.temperature,topP:0.8,maxOutputTokens:p.maxOutputTokens}
      })
    });
    const data=await r.json().catch(()=>({}));
    if(!r.ok){const e=new Error(data?.error?.message||`Gemini API error ${r.status}`);e.status=r.status;throw e;}
    const candidate=data?.candidates?.[0];
    const out=clean(candidate?.content?.parts?.map(x=>x?.text||'').join(''));
    if(!out)throw new Error('AI 未回傳可用文字');
    if(looksIncomplete(out,candidate?.finishReason||'')){
      const e=new Error('AI 回傳內容疑似未完整結束');
      e.status=598;
      throw e;
    }
    return out;
...
function isRetryable(error){
  return error?.name==='AbortError'||[429,500,502,503,504,598].includes(error?.status);
}
...
async function callWithFallback(text,mode,section){
  ...
  for(let i=0;i<MODELS.length;i++){
    ...
    try{
      ...
      console.warn('AI attempt failed',{model,status:error?.status||0,name:error?.name||'',message:String(error?.message||'').slice(0,120)});
...
async function handle(body,forcedMode=''){
  const text=String(body?.text||'').trim();
  const mode=forcedMode||(['summary','needs-assessment','service-evaluation'].includes(body?.mode)?body.mode:'summary');
  ...
  try{
    ...
    if(e?.status===598)return [502,{success:false,error:'AI 回傳內容不完整，系統已嘗試備援模型；請再試一次，原始內容不會遺失。'}];
...
http.createServer((req,res)=>{
  ...
  if(req.method==='GET'&&url.pathname==='/')return send(res,200,{success:true,service:'MUST Resource AI Cloud Run',version:'1.1.0',region:'asia-east1',routes:['POST /ai/isp-summary','POST /ai/polish'],models:MODELS,outputCompletenessCheck:true},origin);
  if(req.method!=='POST'||!['/ai/isp-summary','/ai/polish'].includes(url.pathname))return send(res,404,{success:false,error:'找不到此 API 路徑'},origin);
  if(!allowed(origin))return send(res,403,{success:false,error:'不允許的網站來源'},origin);
  ...
  req.on('end',async()=>{
    ...
    const forcedMode=url.pathname==='/ai/polish'?'service-polish':'';
    const [status,data]=await handle(body,forcedMode);send(res,status,data,origin);
  });
}).listen(PORT,()=>console.log(`MUST Resource AI Cloud Run listening on ${PORT}`));
